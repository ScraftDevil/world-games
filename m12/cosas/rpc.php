...
$query = mysql_query("SELECT total_votes, total_value, used_ips FROM galeria WHERE id='$id_sent' ") or die(" Error: ".mysql_error());
...
$voted=mysql_num_rows(mysql_query("SELECT used_ips FROM galeria WHERE used_ips LIKE '%".$ip."%' AND id='".$id_sent."' "));
...
$newtotals = mysql_query("SELECT total_votes, total_value, used_ips FROM galeria WHERE id='$id_sent' ")or die(" Error: ".mysql_error());
